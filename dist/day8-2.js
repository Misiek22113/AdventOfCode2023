...
while (startNodes.filter((node) => {
    ...
    if (direction == "L") {
        startNodes.map((node) => {
            ...
            let actualNode = network.find((newNode) => newNode.actual === nodeToFind);
            if (!node.isFound) {
                node.steps += 1;
            }
            ...
            tempNodes.push(Object.assign(Object.assign({}, actualNode), { steps: node.steps, isFound: node.isFound }));
        });
    }
    else {
